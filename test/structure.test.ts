...
import { count, jsonLd, startSite, visibleText } from "./harness.ts";
import {
  catalogItems,
  catalogOffers,
  catalogRedirects,
...
addEventListener("unload", () => void site.stop());
...
Deno.test("the catalog page lists exactly the four catalog items", async () => {
...
Deno.test("the navigation has the five agreed links", async () => {
...
Deno.test("each retired catalog slug answers 301 to its new home", async () => {
...
Deno.test("a redirect keeps the query string", async () => {
...
Deno.test("the free audit carries no deadline anywhere", async () => {
  for (
    const path of [
      "/",
      "/catalog",
...
Deno.test("every price in the llms files equals lib/catalog.ts", async () => {
...
Deno.test("every price shown on the home and catalog pages equals lib/catalog.ts", async () => {
  const paths = [
    "/",
    "/catalog",
    ...catalogItems.map((i) => `/catalog/${i.slug}`),
...
Deno.test("the home page has at most six sections and one primary call to action", async () => {
  const html = await site.html("/");
...
Deno.test("the label leads and 'fractional CTO' is only the Ongoing service", async () => {
  const html = await site.html("/");
...
Deno.test("how-i-work has at most five FAQ entries and the JSON-LD comes from them", async () => {
...
Deno.test("each catalog page carries the Offers built from lib/catalog.ts", async () => {
  for (const item of catalogItems) {
...
Deno.test("no internal link answers 404 or points at a redirected slug", async () => {
